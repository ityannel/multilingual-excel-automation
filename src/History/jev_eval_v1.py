...
def ask_jev_amount(office, amount, content):
...
tests = pd.read_csv("tests/jev_amount_tests_other.csv", dtype={"amount": str})
...
for run in range(1, RUNS + 1):
    ...
    for case_id, office, amount, content in zip(tests["case_id"], tests["office"], tests["amount"], tests["content"]):
        ...
        if ambiguous:
            choice, confidence = ask_jev_amount(office, amount, content)
            if confidence >= JEV_THRESHOLD:
                if choice == "thousands":
                    final_value = round(rule_value * 1000, 3)
            else:
                needs_review = True

        records.append({
            "run": run, "case_id": case_id, "ambiguous": ambiguous,
...
summary = results.groupby("run").agg(
...
with pd.ExcelWriter(f"output/jev_eval_other_{run_time}.xlsx") as writer:
    summary.to_excel(writer, sheet_name="Summary")
    results.to_excel(writer, sheet_name="All runs", index=False)
print(f"Saved: output/jev_eval_other_{run_time}.xlsx")